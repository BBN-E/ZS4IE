...
def main(args):
        ...
        if not model_seq[-1][-1]: # if the model_seq does not end in a barrier, it's just a regular sequence
            for i, document in enumerate(intermediate_input_doc_list):
                ...
                for model, _ in model_seq:
                    ...
                    # if "requires_external_argument" in model.__dict__.keys(): # in case external argument file has been supplied
                    #     #logger.info('model {}'.format(str(model)))
                    #     #logger.info('model.__dict__ {}'.format(str(model.__dict__)))
                    #     ret = model.process(document, argument_files[i])
                    # else: ret = model.process(document)
    ...
    logger.info('Start of writing SerifXML files')
    start = time.time()
    for document in intermediate_output_doc_list:
        filename = document.docid + ".xml"
        output_file = os.path.join(args.output_directory, filename)
        document.save(output_file)
    end = time.time()
    logger.info('End of writing SerifXML files')
    logger.info('SerifXML writing took %s seconds', end - start)
